...
def detect(save_img=False):
    out, source, weights, view_img, save_txt, imgsz, outfn = \
        opt.output, opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.outfn
    ...
    if webcam:
        ...
        save_img = True
        dataset = LoadImages(source, img_size=imgsz, do_clahe=opt.clahe)
    ...
    finl_results.image_id = [x.strip('inference/output/') for x in finl_results.image_id]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/exp32_yolov5x-navid-single-1024-scratch-finetune/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='../../input/test/', help='source')  # file/folder, 0 for webcam
    ...
    parser.add_argument('--outfn', type=str , default='submission.csv', help='result file name')
